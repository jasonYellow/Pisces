...
int main(int argc, char *argv[]) {
  ...
  /* Set the URI to play */
  g_object_set (data.playbin2, "uri", "https://example-files.online-convert.com/video/webm/example_small.webm", NULL);
      ...
      if (data.playing) {
        ...
        if (data.seek_enabled && !data.seek_done && current > 5 * GST_SECOND) {
          ...
          gst_element_seek_simple (data.playbin2, GST_FORMAT_TIME,
              GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_KEY_UNIT, 15 * GST_SECOND);
          data.seek_done = TRUE;
        }
      }
    }
